# Erdős Problem 314 — drafting inputs
...
<title>314 | Erdős Problems</title>
<meta property="og:title" content="314 | Erdős Problems">
...
<div class="container">
  ...
  This is true, and shown by Lim and Steinerberger \cite{LiSt24}, who further proved that, for any $\delta&#62;0$, there exist infinitely many $n$ and $m$ such that\[n^2\left\lvert \sum_{n\leq k\leq m}\frac{1}{k}-1\right\rvert\ll \frac{1}{(\log n)^{5/4-\delta}}.\]Erd\H{o}s and Graham (and also Lim and Steinerberger) believe that the exponent of $2$ is best possible here, in that $\liminf \epsilon(n) n^{2+\delta}=\infty$ for all $\delta&#62;0$.
  ...
  [LiSt24] J. Lim and Steinerberger, S., <i>On differences of two harmonic numbers</i>. arXiv:2405.11354 (2024).
  
  </div>
                                        
  <div class="problem-additional-text" style="text-align:center; padding: 10px;">
  <a href="/314">Back to the problem</a>
  </div>
...
## Hosted theorem — plby (https://github.com/plby/lean-proofs/blob/main/src/v4.29.1/ErdosProblems/Erdos314.lean)
    ...
    have h_integral : ∀ u ∈ Set.Icc (-1 / 2 : ℝ) 0, abs ((Real.log (1 + u)) - u + u ^ 2 / 2)
      ...
      have h_ftc : ∫ t in (u)..0, (1 / (1 + t) - 1 + t) = Real.log (1 + 0) - 0 + 0^2 / 2 - (Real.log
        ...
        · exact
            ContinuousOn.intervalIntegrable
              (by
                exact continuousOn_of_forall_continuous
```

## Hosted theorem — jayyhk (https://github.com/Jayyhk/erdos-lean/blob/main/problems/314/Erdos314.lean)
...
lemma bddBelow_eps :
    IsBoundedUnder (· ≥ ·) atTop (fun n : ℕ => (n : ℝ) ^ 2 * eps n
```
